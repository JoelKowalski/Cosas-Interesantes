...
import ubahn.joel.ubahnproject.remote.AsyncTaskDameCabeceras;
...
public class NavigationMenuListener implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void abrirActivityLineas(){
        ...
        try {
          cabeceras= (ArrayList<String>) new AsyncTaskDameCabeceras(context).execute().get();
        } catch (InterruptedException e) {
            e.printStackTrace();
        } catch (ExecutionException e) {
            e.printStackTrace();
        }
        i.putExtra("cabeceras",cabeceras);
        a.startActivity(i);
    }
